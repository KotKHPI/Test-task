<x-layout>
    ...
    <form action="{{ route('books.index') }}" method="GET" class="mb-8 flex items-center space-x-2">
        <label>
            <input type="text" name="search" placeholder="Search by book/author"
                   value="{{ request('search') }}" class="shadow-sm appearance-none border w-full py-2 px-3 text-slate-700 leading-tight focus:outline-none rounded-md border-slate-300 h-10" />
        </label>
